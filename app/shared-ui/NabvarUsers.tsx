...
// Tipado del componente ListItem
...
export default function Navbar() {
  ...
  const getFirstNameAndSurname = (name: string) => {
          ...
          <div className="flex items-center gap-2">
            {imageError || !session.user?.image ? (
              <div className="w-10 h-10 flex items-center justify-center bg-gray-300 rounded-full text-xl text-white">
                {getFirstNameAndSurname(session.user?.name || "NA")}
              </div>
            ) : (
              <Image
            ...
            <span className="dark:text-zinc-100">
              {getFirstNameAndSurname(session.user?.name || "Usuario")}
            </span>
          </div>
        ) : (
          <div className="flex items-center gap-2">
